...
- **UI:** `InterviewApp` — a `TabPane` with an **"Entrevista"** tab (per-question panels, live transcription into the candidate column, inline AI analysis, star rating, **interactive multi-turn follow-up rounds** — the AI proposes 3 follow-ups as radio options; picking one opens a new answer sub-panel that captures the next round, and re-analysis scores the whole exchange — and plain-text session persistence) and a **"Métricas"** tab (per-API usage from `org.example.usage`).
...
- The Groq Whisper *model* is user-selectable via the `GroqWhisperModel` enum (a "Modelo Whisper" dropdown shown only when Groq is the STT engine): `TURBO` (`whisper-large-v3-turbo`, the fast default) vs `LARGE_V3` (`whisper-large-v3`, more accurate on strong accents, a bit slower — same audio-second quota). `createSttProvider` passes the chosen `modelId()` to `GroqWhisperProvider`. Vosk models (small vs large) are *not* accent-robust regardless of size, so heavy accents need Whisper (Groq large-v3 / Gemini), or a future local whisper.cpp provider.
...
**Cross-API usage tracking (`org.example.usage`).** Every paid call reports a `UsageEvent(provider, ApiKind, requests, secondary, RateLimit)` — STT providers (`setUsageListener`) and the LLM client (`GroqClient.setUsageListener`, static) fire one per response, marshalled to the FX thread. `ApiKind` sets the secondary unit (STT = audio-seconds, LLM = tokens) and picks the right `x-ratelimit-*-<unit>` header; `RateLimit.parse(headers, kind)` reads *remaining* quota (Groq + Cerebras send these headers; **Gemini's compat layer does not**, so it yields `RateLimit.empty()` — usage counters only). `UsageTracker` (FX-thread, session-scoped, resets on restart) accumulates per `(provider, kind)`; the "Métricas" tab renders all rows, and two live indicators (the Groq STT gauge in the "Modelo Whisper" row, and a compact LLM indicator by the "Análise" combo) show the active providers. The `req/dia` bar is reliable; the STT `audio-seconds` and any Gemini *remaining* depend on headers that may be absent.
...
Both prompts instruct the model to end with a `FOLLOW-UP QUESTIONS:` section and a final `RATING: n/max` line. `org.example.llm.AnalysisResult` (a `record(prose, score, max, followUps)`) owns that parse: a single `AnalysisResult.parse(rawReply, fallbackMax)` call replaces the four strip/parse helpers the UI used to chain by hand, and `AnalysisResult.stars()` renders the star bar + level band. `QuestionPanel.analyze()` just reads `.prose()` / `.stars()` / `.followUps()`. **Change either prompt's tail format and you must change `AnalysisResult`'s `RATING_PATTERN` / follow-up header patterns**, or the star rating / follow-up radios silently break. The follow-up header is located in two tiers (`findFollowUpHeader`): `FOLLOWUP_HEADER_STRICT` (marker at line start, markdown-tolerant) then `FOLLOWUP_HEADER_LOOSE` — a fallback for reasoning models (Gemini/GPT-OSS) that prefix a lead-in like "Here are three follow-up questions:"; the loose pattern accepts leading words only when the line ends in a colon, so a mid-prose mention ending in a period is never mistaken for the header. `parseFollowUps` and `stripFollowUpSection` share that locator so they can't disagree about where the section starts.
